...
static unsigned char container_find(char *id)
{
	unsigned char i;
	for (i = 0; i < containerCount; ++i)
		if (!strcmp(container[i].container_id, id))
			return i;
	return 255;
}
...
void container_root()
{
   if (argc > 2) // wait, there's more
   {
      if (!strcmp(argv[1], "create"))
	     ...
	     container_stop( argv[2] );
	  else if (!strcmp(argv[1], "start"))
	     container_start( argv[2] );
...
{
	unsigned char i = container_find(id);
	if (i == 255) { cputs("no such container.\r\n"); return; }
	container[i].status = 1;
	cprintf("container [%s] stopped\r\n", container[i].container_id);
}
...
void container_start( char *id )
{
	unsigned char i = container_find(id);
	if (i == 255) { cputs("no such container.\r\n"); return; }
	container[i].status = 0;
	cprintf("container [%s] started\r\n", container[i].container_id);
}
...
void container_rm( char *id )
{
	...
	i = container_find(id);
	if (i == 255) { cputs("no such container.\r\n"); return; }
	cprintf("container [%s] removed\r\n", container[i].container_id);
	for (j = i; j < containerCount - 1; ++j)
